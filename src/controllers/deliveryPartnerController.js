...
const getDeliveryPartnerOrders = async (req, res) => {
  try {
    ...
    const orders = await prisma.order.findMany({
      ...
      // Include related Customer and Restaurant data, selecting only the 'name' field
      include: {
        customer: {
          select: {
            name: true // Select only the name field from the Customer model
          }
        },
        restaurant: {
          select: {
            name: true // Select only the name field from the Restaurant model
          }
        }
      }
    });

    // Transform the orders to flatten the customer and restaurant names
    const transformedOrders = orders.map(order => ({
      ...order,
      customerName: order.customer ? order.customer.name : 'N/A', // Extract customer name, default to 'N/A' if not found
      restaurantName: order.restaurant ? order.restaurant.name : 'N/A', // Extract restaurant name, default to 'N/A' if not found
